...
export const ConfigSchema = z.object({
  ...
  llm: z
    .object({
      ...
      auxModel: z.string().optional().describe('Cheaper model in "provider/model" format used for auxiliary/control turns (todo-check, max-steps summary, history gist, compaction). Falls back to the primary model when unset. These turns are constrained instruction-following tasks, not full conversational turns, so a smaller/cheaper model is usually sufficient. It is NOT used for the finalise turn, which is a compute turn and runs on the agent\'s main model unless that agent sets its own finaliseModel.'),
      gistStructuredOutput: z.boolean().optional().describe('Whether history-gist generation may use native structured outputs (generateObject / JSON-schema response_format). Default true. Set false when your gist model (usually auxModel) does not support the responseFormat feature — e.g. MiniMax — to skip the structured attempt and go straight to the text+JSON-parse fallback, avoiding a wasted call per turn. On a structured failure the model is auto-marked for the text fallback for the rest of the process regardless of this flag.'),
      ...
      resumeTurnsOnRestart: z.boolean().optional().describe('Recover in-flight agent turns after a restart. When on, each user-initiated turn is recorded before it runs and cleared when it completes; a turn still marked in-flight on startup is re-driven, replaying already-executed steps (from conversation_steps) so it continues rather than restarts. Off by default. Telegram turns only for now.'),
      resumeTurnsMaxAttempts: z.number().int().min(1).max(10).optional().describe('Max times startup recovery will retry a single in-flight turn before abandoning it, so a turn that reliably crashes the process cannot crash-loop (default 3). Only used when resumeTurnsOnRestart is enabled.'),
      resumeTurnsMaxAgeMinutes: z.number().int().min(1).max(1440).optional().describe('Only resume in-flight turns interrupted within this many minutes of startup — a spurious restart recovery is seconds-to-minutes old (default 10). Older rows are swept (cleared, not resumed), so a leaked or stale record can never trigger a surprise replay days later. Only used when resumeTurnsOnRestart is enabled.'),
      resumeTurnsMaxRecovered: z.number().int().min(1).max(200).optional().describe('Global cap on how many turns startup recovery will resume in one boot (default 20). After the recency and per-chat-dedup filters, if more turns still survive than this, only the newest are resumed and the rest are swept — a backstop so a pathological pile-up cannot flood every chat at once. Only used when resumeTurnsOnRestart is enabled.'),
  ...
  tools: z
    .object({
      ...
      deferredTools: z
        .boolean()
        .optional()
        .describe('On-demand tool loading (default false). When true, the model receives a compact directory of the available deferred tool families plus a core set and the search_tools/load_tools meta-tools. Full schemas stay withheld until the model loads exact tool names. Also enabled by the DEFERRED_TOOLS env var. Best paired with defaultProfile "full" so every tool is discoverable and loadable.'),
      loopBreaker: z
        .object({
          enabled: z.boolean().optional().describe('Block degenerate repeated tool calls (default true). When a call is blocked it is not executed; the model receives a structured tool error telling it to change approach.'),
          repeats: z.number().int().min(2).max(50).optional().describe('How many identical (tool, input) calls within the window trip the breaker (default 3). The first repeats-1 calls execute normally; the next one is blocked.'),
          window: z.number().int().min(2).max(200).optional().describe('How many recent calls the breaker looks back over (default 6). A sliding window rather than a consecutive run, so alternating A-B-A-B loops are caught too. Values below `repeats` are raised to it, since a shorter window could never trip.'),
          requireIdenticalOutput: z.boolean().optional().describe('Only trip when every prior occurrence also returned the same output (default true). Keeps a legitimate read-edit-read cycle from being blocked, at the cost of never tripping on tools whose output embeds a timestamp or nonce. Set false to match on input alone.'),
          outputHashChars: z.number().int().min(0).max(1_000_000).optional().describe('Hash only the first N characters of tool output when comparing (default 0 = hash the full payload). Set this to ignore a volatile tail, e.g. a trailing timestamp, that would otherwise stop identical calls from being recognised as identical.'),
        })
        .optional()
